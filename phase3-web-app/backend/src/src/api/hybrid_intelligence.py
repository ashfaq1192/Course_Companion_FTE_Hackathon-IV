...
def generate_new_adaptive_path(
    ...
    service = HybridIntelligenceService(db)
    path = service.generate_adaptive_learning_path(
...
def grade_assessment_with_llm(
    ...
    service = HybridIntelligenceService(db)
    assessment = service.grade_assessment_with_llm(
        user_id=current_user.id,
        quiz_attempt_id=UUID(request_data["quiz_attempt_id"]),
